...
static int log_parse_options(struct log_state *s, struct log_options *opt, int argc, char **argv);
...
int gitprefix(log)(int argc, char *argv[])
{
	...
	last_arg = log_parse_options(&s, &opt, argc, argv);
	if (last_arg == -1) {
...
static int log_usage(const char *message, const char *arg)
{
	if (message && arg)
		fprintf(stderr, "%s: %s\n", message, arg);
	else if (message)
	...
	struct log_state *s, struct log_options *opt, int argc, char **argv)
{
	...
	for (args.pos = 1; args.pos < argc; ++args.pos) {
		...
		if (a[0] != '-') {
			...
			return log_usage("Unsupported argument", a);
	}
