...
static int object_database(git_repository *repo, git_oid *oid);
static int commit_writing(git_repository *repo);
static int commit_parsing(git_repository *repo);
static int tag_parsing(git_repository *repo);
static int tree_parsing(git_repository *repo);
static int blob_parsing(git_repository *repo);
static int revwalking(git_repository *repo);
static void index_walking(git_repository *repo);
static void reference_listing(git_repository *repo);
static int config_files(const char *repo_path, git_repository *repo);
 ...
 * as an example.
 */
static int check_error(int error_code, const char *action)
{
...
int gitprefix(general) (int argc, char** argv)
{
	int error;
	git_oid oid;
	char *repo_path;
	...
	pi(oid_parsing(&oid));
	pi(object_database(repo, &oid));
	pi(commit_writing(repo));
	pi(commit_parsing(repo));
	pi(tag_parsing(repo));
	pi(tree_parsing(repo));
	pi(blob_parsing(repo));
	pi(revwalking(repo));
	pi(index_walking(repo));
	pi(reference_listing(repo));
	pi(config_files(repo_path, repo));
...
static int object_database(git_repository *repo, git_oid *oid)
{
	...
	gitret(check_error(error, "finding object in repository"));
...
static int commit_writing(git_repository *repo)
{
	 ...
	 */
	giterr(git_signature_new(&author,
			"Scott Chacon", "schacon@gmail.com", 123456789, 60));
	giterr(git_signature_new(&committer,
	 ...
	 */
	gitret(git_oid_fromstr(&tree_id, "f60079018b664e4e79329a7ef9559c8d9e0378d1"));
	gitret(git_tree_lookup(&tree, repo, &tree_id));
	gitret(git_oid_fromstr(&parent_id, "5b5b025afb0b4c913b4c338a42934a3863bf3644"));
	gitret(git_commit_lookup(&parent, repo, &parent_id));
	visit(parent);
...
static int commit_parsing(git_repository *repo)
{
	...
	gitret(check_error(error, "looking up commit"));
...
static int tag_parsing(git_repository *repo)
{
	...
	gitret(check_error(error, "looking up tag"));
...
static int tree_parsing(git_repository *repo)
{
	...
	gitret(git_tree_lookup(&tree, repo, &oid));
...
static int blob_parsing(git_repository *repo)
{
	...
	gitret(git_blob_lookup(&blob, repo, &oid));
...
static int revwalking(git_repository *repo)
{
	...
	while ((git_revwalk_next(&oid, walk)) == 0) {
		...
		gitret(check_error(error, "looking up commit during revwalk"));
...
static int config_files(const char *repo_path, git_repository* repo)
{
	...
	gitret(check_error(git_config_open_ondisk(&cfg, config_path), "opening config"));

	if (git_config_get_int32(&autocorrect, cfg, "help.autocorrect") == 0)
		printf("Autocorrect: %d\n", autocorrect);

	gitret(check_error(git_repository_config_snapshot(&snap_cfg, repo), "config snapshot"));
	...
	switch (error_code)
	{
		...
		case GIT_ENOTFOUND:
			...
			break;
		default:
			gitret(check_error(error_code, "get_int32 failed"));
	}
	git_config_free(cfg);
	...
	switch (error_code)
	{
		...
		case GIT_ENOTFOUND:
			...
			break;
		default:
			gitret(check_error(error_code, "get_string failed"));
	}
