...
#if defined(__APPLE__) || defined(_OS_ANDROID_)
...
void change_file_date(const char *filename, uLong dosdate, tm_unz tmu_date)
{
	...
	HANDLE hFile;
	FILETIME ftm, ftLocal, ftCreate, ftLastAcc, ftLastWrite;
	size_t wcharCount = strlen(filename);
	wchar_t *towstr_buf = (wchar_t *)malloc(sizeof(wchar_t)*wcharCount);
	mbstowcs(towstr_buf, filename, wcharCount);
	hFile = CreateFile2(towstr_buf, GENERIC_READ | GENERIC_WRITE, 0, OPEN_EXISTING, NULL);
	free(towstr_buf);
	//TODO : Implement.
	/*GetFileTime(hFile,&ftCreate,&ftLastAcc,&ftLastWrite);
	DosDateTimeToFileTime((WORD)(dosdate>>16),(WORD)dosdate,&ftLocal);
...
int do_extract_onefile(unzFile uf,
	const char* filename, int opt_extract_without_path,
	int opt_overwrite, const char* password)
{
	int err = UNZ_OK;
