...
uLong filetime(const char *filename, tm_zip *tmzip, uLong *dostime)
{
	...
	hFind = FindFirstFileExA(filename, FindExInfoStandard, &ff32,
...
int main(int argc, char *argv[])
{
	...
	strcat(zipfilename, argv[zipfilenamearg]);
	...
	for (i = zipfilenamearg + 1; (i < argc) && (err == ZIP_OK); i++)
	{
		...
		const char *filenameinzip_relative = argv[i];
		char filenameinzip[MAXFILENAME] = { 0 };
		strcat(filenameinzip, filenameinzip_relative);
		...
		savefilenameinzip = filenameinzip_relative;
