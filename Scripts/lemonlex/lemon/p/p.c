...
/* The regex actions */
static void lx_state_action_2(lxLexer *lxpLexer){ printf("Recieved EOF\n"); }
static void lx_state_action_4(lxLexer *lxpLexer){ printf("COMMENT_%s: %s\n", "START", (lxpLexer->token).buf); }
static void lx_state_action_5(lxLexer *lxpLexer){ printf("HD_%s: %s\n", "START", (lxpLexer->token).buf); }
...
static void lx_state_action_8(lxLexer *lxpLexer){ printf("COMMENT_%s: %s\n", "END", (lxpLexer->token).buf); }
...
static void lx_state_action_12(lxLexer *lxpLexer){ printf("HD_%s: %s\n", "CONTENTS", (lxpLexer->token).buf);}
static void lx_state_action_13(lxLexer *lxpLexer){ printf("HD_%s: %s\n", "END", (lxpLexer->token).buf); }
static void lx_state_action_14(lxLexer *lxpLexer){ printf("HD_%s: %s\n", "CONTENTS", (lxpLexer->token).buf);}
...
S10:
  printf("S10(%d,%d):0x%X(%c)\n", lxpLexer->lxrow, lxpLexer->lxcol, ch, ((ch>= 32) && (ch <= 127))?ch:' ');
  goto S9; /* by goto_state */
S11:
  printf("S11(%d,%d):0x%X(%c)\n", lxpLexer->lxrow, lxpLexer->lxcol, ch, ((ch>= 32) && (ch <= 127))?ch:' ');
  goto S10; /* by goto_state */
S12:
  printf("S12(%d,%d):0x%X(%c)\n", lxpLexer->lxrow, lxpLexer->lxcol, ch, ((ch>= 32) && (ch <= 127))?ch:' ');
...
S13:
  printf("S13(%d,%d):0x%X(%c)\n", lxpLexer->lxrow, lxpLexer->lxcol, ch, ((ch>= 32) && (ch <= 127))?ch:' ');
  if((ch == 0) && (is_final == 0)) {lxpLexer->lxstate=13;return 0;}
  LX_ACTION(13);/*NO_SEND*/LX_RESET;
  if(ch == 0) {LX_SEND(0);return 0;}
  if(LX_LEAVE_NESTING() == 1){goto L0;}
  goto S12; /* by next_state */
S14:
  printf("S14(%d,%d):0x%X(%c)\n", lxpLexer->lxrow, lxpLexer->lxcol, ch, ((ch>= 32) && (ch <= 127))?ch:' ');
...
S15:
  printf("S15(%d,%d):0x%X(%c)\n", lxpLexer->lxrow, lxpLexer->lxcol, ch, ((ch>= 32) && (ch <= 127))?ch:' ');
  goto S14; /* by goto_state */
S16:
  printf("S16(%d,%d):0x%X(%c)\n", lxpLexer->lxrow, lxpLexer->lxcol, ch, ((ch>= 32) && (ch <= 127))?ch:' ');
